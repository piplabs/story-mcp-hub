...
class TestStoryService:
    ...
    def mock_story_client(self):
        ...
        mock_client.License.getLicenseTerms = Mock(return_value=get_mock_license_terms())
        ...
        mock_client.IPAsset.mintAndRegisterIpAssetWithPilTerms = Mock(return_value=get_mock_mint_and_register_response())
                    ...
                    with patch("services.story_service.create_address_resolver") as mock_resolver_fn:
                        ...
                        with patch("services.story_service.get_contracts_by_chain_id") as mock_contracts_fn:
                            ...
                            service = StoryService(
                                ...
                                network="mainnet"  # Explicitly set network to avoid chain_id detection
                            )
                    ...
                    with patch("services.story_service.create_address_resolver") as mock_resolver_fn:
                        ...
                        with patch("services.story_service.get_contracts_by_chain_id") as mock_contracts_fn:
                            ...
                            service = StoryService(
                                rpc_url="https://mainnet.storyrpc.io",
                                private_key="e432d83208d8575baf3b53b37077fca1e41e8d2e1587d127d6e43e90ed1e800a",
                                network="mainnet"  # Explicitly set network to avoid auto-detection
                            )
                            
                            # Verify web3 was initialized correctly
                            mock_web3_class.HTTPProvider.assert_called_once_with("https://mainnet.storyrpc.io")
                            ...
                            assert service.chain_id == CHAIN_IDS["mainnet"]
    ...
    def test_get_license_terms(self, story_service, mock_story_client):
        ...
        mock_story_client.License.getLicenseTerms.assert_called_once_with(SAMPLE_LICENSE_TERMS_ID)
    ...
    def test_attach_license_terms(self, story_service, mock_story_client):
        ...
        assert kwargs["license_terms_id"] == SAMPLE_LICENSE_TERMS_ID
        assert kwargs["license_template"] == "0x2E896b0b2Fdb7457499B56AAaA4AE55BCB4Cd316"  # Default LICENSE_TEMPLATE
        ...
        mock_tx_receipt = {
            ...
            "txReceipt": {
                "logs": [
                    {
                        "topics": [
                            mock_web3.keccak(text="DisputeRaised(uint256,address,bytes32,string)"),
